// The maximum number of entries in the hardware's scatter/gather list. I am making this so large for a few reasons:
//     1) we're faking this with uncompressed surfaces -- these are large buffers which will map to a lot of s/g entries
//     2) the fake hardware implementation requires at least one frame's worth of s/g entries to generate a frame
#define SCATTER_GATHER_MAPPINGS_MAX 128

// This structure is used to keep the scatter gather table for the fake hardware as a doubly linked list.
...
private:
    // The image synthesizer. This is a piece of code which actually draws the requested images.
    CImageSynthesizer *m_ImageSynth{};

    // The synthesis buffer. This is a private buffer we use to generate the capture image in.
    // The fake "scatter / gather" mappings are filled in from this buffer during each interrupt.
    ...
    // Lookaside for memory for the scatter / gather entries on the scatter / gather list.
    ...
    // The "Interrupt Time". Number of "fake" interrupts that have occurred since the hardware was started.
...
public:
    ...
    // This is the free callback for the bagged hardware sim. Not providing one will call ExFreePool, which is not what
    // we want for a constructed C++ object. This simply deletes the simulation.
    static void Cleanup(IN CHardwareSimulation *HwSim) {
        delete HwSim;
    }

    // Called from the simulated interrupt. First we fake the hardware's actions (at DPC) then we call the "Interrupt
    // service routine" on the hardware sink.
    void FakeHardware();

    // "Start" the fake hardware.  This will start issuing interrupts and DPC's.
    // The frame rate, image size, and a synthesizer must be provided.
    NTSTATUS Start(CImageSynthesizer *ImageSynth, IN LONGLONG TimePerFrame, IN ULONG Width, IN ULONG Height,
                   IN ULONG ImageSize);

    // "Pause" or "unpause" the fake hardware.  This will stop issuing interrupts or DPC's on a pause and restart them
    // on an unpause. Note that this will not reset counters as a Stop() would.
    NTSTATUS Pause(IN BOOLEAN Pausing);

    // "Stop" the fake hardware. This will stop issuing interrupts and DPC's.
    NTSTATUS Stop();

    // Program a series of scatter gather mappings into the fake hardware.
    ULONG ProgramScatterGatherMappings(IN PKSSTREAM_POINTER Clone, IN PUCHAR *Buffer, IN PKSMAPPING Mappings,
