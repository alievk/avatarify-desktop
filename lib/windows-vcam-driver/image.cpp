...
// g_FontData:
// The following is an 8x8 bitmapped font for use in the text overlay code.
...
void CImageSynthesizer::SynthesizeBars() {
    /*++
    Routine Description:
        Synthesize EIA-189-A standard color bars onto the Image. The image in question is the current synthesis buffer.
    Arguments:
        None
    Return Value:
        None
...
void CImageSynthesizer::OverlayText(
        _In_ ULONG LocX,
        _In_ ULONG LocY,
        _In_ ULONG Scaling,
        _In_ LPSTR Text,
    ...
    for (ULONG row = 0; row < 8 && SpaceY; row++) {
        ...
        while (CurChar && *CurChar) {
            ...
            // Separate each character by one space. Account for the border space at the end by placing the separator
            // after the last character also.
#ifndef NO_CHARACTER_SEPARATION
            if (CurSpaceX) {
                PutPixel(BgColor);
                CurSpaceX--;
        ...
        // If there is no separation character defined, account for the border.
