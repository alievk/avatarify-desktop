...
void SimulatedInterrupt(IN PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArg1, IN PVOID SystemArg2) {
    auto *HardwareSim = (CHardwareSimulation *) DeferredContext;
    if (HardwareSim) {
...
CHardwareSimulation::CHardwareSimulation(IN IHardwareSink *HardwareSink, PUCHAR inBuffer) :
    ...
    // Initialize the DPC's, timer's, and locks necessary to simulate this capture hardware.
...
NTSTATUS CHardwareSimulation::Start(IN CImageSynthesizer *ImageSynth, IN LONGLONG TimePerFrame, IN ULONG Width,
    ...
    m_SynthesisBuffer = reinterpret_cast <PUCHAR> (ExAllocatePoolZero(NonPagedPoolNx, m_ImageSize, AVSHWS_POOLTAG));
...
NTSTATUS CHardwareSimulation::Pause(BOOLEAN Pausing) {
    ...
    if (Pausing && m_HardwareState == HardwareRunning) {
        ...
        // For unpausing the hardware, we need to compute the relative time and restart interrupts.
...
NTSTATUS CHardwareSimulation::Stop() {
    ...
    while (m_ScatterGatherMappingsQueued > 0) {
        ...
        auto SGEntry = reinterpret_cast <PSCATTER_GATHER_ENTRY> (
                CONTAINING_RECORD (listEntry, SCATTER_GATHER_ENTRY, ListEntry)
        );
...
ULONG CHardwareSimulation::ProgramScatterGatherMappings(IN PKSSTREAM_POINTER Clone, IN PUCHAR *Buffer,
                                                        IN PKSMAPPING Mappings, IN ULONG MappingsCount,
...
NTSTATUS CHardwareSimulation::FillScatterGatherBuffers() {
    ...
    while (BufferRemaining && m_ScatterGatherMappingsQueued > 0 && m_ScatterGatherBytesQueued >= BufferRemaining) {
        ...
        if (SGEntry->CloneEntry->StreamHeader->Size >= sizeof(KSSTREAM_HEADER) + sizeof(KS_FRAME_INFO)) {
            auto FrameInfo = reinterpret_cast <PKS_FRAME_INFO> (SGEntry->CloneEntry->StreamHeader + 1);
